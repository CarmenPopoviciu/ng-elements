...
export class Cppn {
  ...
  generateWeights(neuronsPerLayer: number, weightsStdev: number) {
    ...
    for (let i = 0; i < MAX_LAYERS; i++) {
      this.intermediateWeights.push(
        dl.Array2D.randTruncatedNormal(
  ...
  setNumLayers(numLayers: number = 3) {
    this.numLayers = numLayers;
  }

  setZ1Scale(z1Scale: number = 100) {
    this.z1Scale = z1Scale;
  }

  setZ2Scale(z2Scale: number = 100) {
  ...
  private async runInferenceLoop() {
    ...
    const lastOutput = math.scope(() => {
      ...
      const latentVars = math.concat2D(
        ...
        math.multiply(z2, this.ones) as dl.Array2D,
        concatAxis
      );

      const activation = (x: dl.Array2D) =>
        activationFunctionMap[this.activationFunction](x);

      let _lastOutput: dl.NDArray = math.concat2D(
        this.inputAtlas,
        latentVars,
        concatAxis
      );
      _lastOutput = activation(
        math.matMul(_lastOutput as dl.Array2D, this.firstLayerWeights)
      );

      for (let i = 0; i < this.numLayers; i++) {
        const matmulResult = math.matMul(
          _lastOutput as dl.Array2D,
          this.intermediateWeights[i]
        );
      ...
      return math
        .sigmoid(math.matMul(_lastOutput as dl.Array2D, this.lastLayerWeights))
        .reshape([this.canvas.height, this.canvas.width, 3]);
    });

    await renderToCanvas(lastOutput as dl.Array3D, this.canvas);
